...
export class WorkflowAgent {
  ...
  async run<TSchemaDef extends TSchema | undefined = undefined>(
    ...
    // GOVERNANCE: gate the EFFECTIVE model — the exact one createAgentSession will
    // spawn — through agent-dispatch BEFORE the in-process spawn. Precedence MUST
    // mirror the spawn options below (resolvedModel wins via the later spread, else
    // sessionOptions.model, else the session default the SDK inherits from
    // settingsManager.getDefaultModel()). Gating only the requested `modelSpec`
    // missed two real bypasses: (1) an inherited forbidden session default when the
    // agent is untagged, and (2) a forbidden `sessionOptions.model` — both reach the
    // spawn ungated. Throws the NON-recoverable GovernanceDenied for forbidden
    // (haiku) / override-only (sonnet) models, fail-closed if the guard is
    // unreachable for a governed disposition. This throw is OUTSIDE the try/finally
    // below, so a denied spawn is never written to the outcome ledger. The spawn
    // stays in-process → journaled resume + getSessionStats cost survive.
    const sessionOptionModel = this.sessionOptions.model as
      | { provider?: string; id?: string }
      | undefined;
    const sessionOptionSpec = sessionOptionModel?.id
      ? sessionOptionModel.provider
        ? `${sessionOptionModel.provider}/${sessionOptionModel.id}`
        : sessionOptionModel.id
      : undefined;
    const settingsWithDefault = settingsManager as unknown as {
      getDefaultModel?: () => string | null | undefined;
      getDefaultProvider?: () => string | null | undefined;
    };
    // Treat the saved default as usable ONLY when BOTH provider and id are set —
    // this is exactly the SDK's own branch-1 condition (model-resolver.js:
    // `if (defaultProvider && defaultModelId)`). When only one is set the SDK
    // ignores the saved default and falls back to defaultModelPerProvider /
    // availableModels[0]; we surface that as an undeterminable spec → governModelSpec
    // fail-closes, rather than gating an id the SDK will not actually spawn.
    const savedProvider = settingsWithDefault.getDefaultProvider?.();
    const savedModelId = settingsWithDefault.getDefaultModel?.();
    const sessionDefault = savedProvider && savedModelId ? `${savedProvider}/${savedModelId}` : undefined;
    const effectiveSpec = effectiveModelSpec({
      resolvedSpec: resolvedModel ? `${resolvedModel.provider}/${resolvedModel.id}` : undefined,
      sessionOptionSpec,
      sessionDefault,
    });
    ...
    // GOVERNANCE: outcome for the ledger record in finally. Flipped to true only
    // after a result is in hand, so thrown errors are recorded as failures.
    ...
    try {
      ...
      try {
        ...
        usageTokens = { input: tokens.input, output: tokens.output };
        options.onUsage?.({
          input: tokens.input,
      ...
      // GOVERNANCE: record this in-process spawn to the agent-dispatch outcome
      // ledger (the dispatch path never bracketed it). Best-effort; never throws.
      recordOutcome({
        taskClass: governed.taskClass,
        model: resolvedModel ? `${resolvedModel.provider}/${resolvedModel.id}` : (modelSpec ?? null),
        tokensIn: usageTokens?.input,
        tokensOut: usageTokens?.output,
