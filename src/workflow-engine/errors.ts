...
/**
 * VENDOR-MOD: converted from `enum` to an `as const` object + union type so the
 * module loads under Node's `--experimental-strip-types` (the fork's unit-test
 * runtime rejects TS enums). Member access (`WorkflowErrorCode.X`) and the type
 * usage (`code: WorkflowErrorCode`) are unchanged; string values are identical.
 */
export const WorkflowErrorCode = {
  ...
  /**
   * agent-dispatch governance denied the effective model before spawn (forbidden
   * haiku / override-only sonnet / fail-closed guard). NON-recoverable by design:
   * a denial must abort the workflow hard, never be retried into a silent null.
   */
